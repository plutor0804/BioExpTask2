...
class EEGFileHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        if os.path.abspath(event.src_path) == self.filepath:
            print(f"檔案 {self.filepath} 變動偵測到，開始預測")
            predict_and_press(self.filepath)

def main():
    filepath = "../data/bci_dataset_113-2/S01/1.txt"
    print("🧠 EEG 即時預測啟動，監控檔案變動觸發預測")
    print("🎮 手動控制：i=↑ j=← k=↓ l=→")
    start_manual_listener()

    event_handler = EEGFileHandler(filepath)
    observer = Observer()
    observer.schedule(event_handler, path=os.path.dirname(os.path.abspath(filepath)), recursive=False)
    observer.start()
